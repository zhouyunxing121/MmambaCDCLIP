...
def extend_cfg(cfg):
    ...
    # Config for ProText
    cfg.TRAINER.PROTEXT = CN()
    cfg.TRAINER.PROTEXT.N_CTX_VISION = 0  # it must be set to 0 as ProText does not adapt vision side at all
    cfg.TRAINER.PROTEXT.N_CTX_TEXT = 4  # number of context vectors at the language branch
    cfg.TRAINER.PROTEXT.CTX_INIT = "a photo of a"  # initialization words
    cfg.TRAINER.PROTEXT.PREC = "fp16"  # fp16, fp32, amp
    cfg.TRAINER.PROTEXT.PROMPT_DEPTH_VISION = 0  # it must be set to 0 as ProText does not adapt vision side at all
    cfg.TRAINER.PROTEXT.PROMPT_DEPTH_TEXT = 9  # Max 12, Min 0, if 0, it will be using shallow IVLP prompting (J=1)
    cfg.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new
    cfg.TRAINER.PROTEXT.GPT_PATH = "templates/CuPL_image_prompts.json"
    cfg.TRAINER.PROTEXT.USE_TEMPLATES = False
    cfg.TRAINER.PROTEXT.USE_ATTRIBUTE_DATA = False
