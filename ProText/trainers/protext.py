...
class VLPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        #修改---2026-3-30_----见----解决ProText的PyTorch多GPU报错：CUDA设备不匹配
        #self.clip_model_zs = load_clip_to_cpu(cfg, True).float().cuda()
        self.clip_model_zs = load_clip_to_cpu(cfg, True).float() # 先不要 .cuda()
        #修改---2026-3-30_----见----解决ProText的PyTorch多GPU报错：CUDA设备不匹配
    ...
    def forward(self, tokenized_prompts):
        ...
        #修改---2026-3-30_----见----解决ProText的PyTorch多GPU报错：CUDA设备不匹配
        # 【关键修复】将模型移动到输入张量所在的设备
        device = tokenized_prompts.device
        self.clip_model_zs = self.clip_model_zs.to(device) # 动态移动模型到当前 GPU
    ...
    def forward(self, inputs, outputs=None):
        ...
        if self.prompt_learner.training:
            ...
            with torch.no_grad():
                ...
                return logits
    '''
    def forward(self, inputs, outputs=None, return_feature=False):
        """
        新增：return_feature=True → 直接返回 CLIP 图像特征 [B, 512]
        用于 ChangeCLIP 变化检测模型的语义特征提取
        """
        if self.prompt_learner.training:
            tokenized_texts_labels = outputs
            tokenized_texts_inputs = inputs
            ...
            prompts = self.prompt_learner(tokenized_texts_inputs)
            text_features = self.text_encoder(prompts, tokenized_texts_inputs)
            outputs = text_features / text_features.norm(dim=-1, keepdim=True)
            return outputs, target_embed
            ...
            with torch.no_grad():
                ...
                # 如果只需要图像特征，直接返回（给 ChangeCLIP 使用）
                if return_feature:
