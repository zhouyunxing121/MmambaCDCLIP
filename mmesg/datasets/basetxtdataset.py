...
class TXTSegDataset(BaseDataset):
    ...
    def _update_palette(self) -> list:
        ...
        return new_palette   # may be different in each iteration if not specified.
    ...
    def load_data_list(self) -> List[dict]:
        ...
        print(f"Loading data from {img_dir} and {ann_dir}")
...
class TXTCDDatasetJSON(BaseDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        ann_dir = self.data_prefix.get('seg_map_path', None)
        #print(f"Loading data from {img_dir} and {ann_dir}")
        #print(f"Loading data from {img_dir2} and {ann_dir}")
        if osp.isfile(self.ann_file):
            #print(f"ann_file: {self.ann_file}")
            #print("C:\\Users\\gjingx\\Desktop\\2025\\ChangeCLIP\\mmseg\\datasets\\basetxtdataset.py")
            lines = mmengine.list_from_file(
                self.ann_file, backend_args=self.backend_args)
            #print(f"lines: {lines}")
            #print(f"json_path1: {os.path.dirname(lines[0].strip().split('  ')[0])+'_clipcls_56_vit16.json'}")
            #print(f"json_path2: {os.path.dirname(lines[1].strip().split('  ')[1])+'_clipcls_56_vit16.json'}")
            ...
            print("osp.isfile(self.ann_file) is False")
            print(f"Loading images from {img_dir} with suffix {self.img_suffix}")
            self.img_suffix='.png'
            images = fileio.list_dir_or_file(
            dir_path=img_dir,
            list_dir=False,
            suffix=self.img_suffix,
            recursive=True,
