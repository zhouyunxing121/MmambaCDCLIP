...
def cross_selective_scan(
    ...
    if (not dt_low_rank):
        ...
        K = dt_projs_weight.shape[0]
        dts, Bs, Cs = torch.split(x_dbl.view(B, -1, L), [D, K * N, K * N], dim=1)
...
class SS2D(nn.Module):
    def __init__(
        ...
        initialize="v0",
        # ======================
        forward_type="v5",#forward_type="v2",
        channel_first=False,
        # ======================
        **kwargs,
    ):
    ...
    def __initv0__(
        ...
        **kwargs,
        ...
        proj_out_dim = d_inner if ssm_disable_z else d_inner * 2
        self.in_proj = nn.Linear(d_model, proj_out_dim, bias=bias, **factory_kwargs)
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        self.d_inner = int(ssm_ratio * d_model)  # ← 直接赋值给 self.d_inner
        self.ssm_ratio = ssm_ratio

        #d_inner = int(ssm_ratio * d_model)
        # 原有代码：使用 self.d_inner 替代局部变量（可选，但推荐）
        d_inner = self.d_inner  # 保持后续代码兼容性（或直接全局替换 d_inner → self.d_inner）
        dt_rank = math.ceil(d_model / 16) if dt_rank == "auto" else dt_rank
        self.d_conv = d_conv
        ...
        #——修改——2026-4-8————见——————Gemini——ChangeCLIP 项目介绍与解析
        # 改为：
        #explicit_disable_z = kwargs.get("ssm_disable_z", False)
        #postfix_disable_z, forward_type = checkpostfix("noz", forward_type)
        #self.disable_z = explicit_disable_z or postfix_disable_z
        # 修改后长这样：
        explicit_disable_z = ssm_disable_z  # 🚨 修复 Bug：直接使用传入的参数名
        postfix_disable_z, forward_type = checkpostfix("noz", forward_type)
        self.disable_z = explicit_disable_z or postfix_disable_z
        #——修改——2026-4-8————见——————Gemini——ChangeCLIP 项目介绍与解析
        ...
        FORWARD_TYPES = dict(
            ...
            v5=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True,
        ...
        """
        # x_proj & dt_projs & A_logs & Ds (all use k_group=8)
    self.x_proj_weight = nn.Parameter(torch.stack([
        nn.Linear(d_inner, dt_rank + d_state * 2, bias=False, **factory_kwargs).weight
        for _ in range(k_group)
    ], dim=0))  # (K, N, inner)
    ...
    def forwardxv(self, x: torch.Tensor, mode="xv1a", omul=False, **kwargs):
        ...
        if mode in ["xv1a"]:
            ...
            """
            us = CrossScanTriton.apply(us.contiguous()).view(B, 8, -1, L)
    
            dts = CrossScanTriton.apply(dts.contiguous()).view(B, 8, -1, L)
            Bs = CrossScanTriton1b1.apply(Bs.view(B, 8, -1, H, W).contiguous()).view(B, 8, -1, L)
            Cs = CrossScanTriton1b1.apply(Cs.view(B, 8, -1, H, W).contiguous()).view(B, 8, -1, L)
            ...
            """
            us = CrossScanTriton.apply(us.contiguous()).view(B, 8, -1, L)
            dts = CrossScanTriton1b1.apply(dts.view(B, 8, -1, H, W).contiguous()).view(B, 8, -1, L)
            Bs = CrossScanTriton1b1.apply(Bs.view(B, 8, -1, H, W).contiguous()).view(B, 8, -1, L)
            Cs = CrossScanTriton1b1.apply(Cs.view(B, 8, -1, H, W).contiguous()).view(B, 8, -1, L)
            dts = F.conv1d(dts.contiguous().view(B, -1, L), dt_projs_weight.view(K * self.d_inner, self.dt_rank, 1), None, groups=K)
            us, dts = us.contiguous().view(B, -1, L), dts
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.num_patches = (224 // patch_size[0]) * (224 // patch_size[1])  # only for example
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        assert C == self.in_chans, f"Input channel {C} does not match model in_chans {self.in_chans}"
        # Ensure the input size is divisible by patch size
        assert H % self.patch_size[0] == 0 and W % self.patch_size[1] == 0, \
            f"Input image size ({H}*{W}) is not divisible by patch size {self.patch_size}"
    ...
    def flops(self, N):
        # N: num_patches = H*W / patch_size**2
        flops = 0
        # conv
        flops += N * self.embed_dim * self.in_chans * (self.patch_size[0] * self.patch_size[1])
        # norm
        if self.norm is not None:
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.ssm_disable_z = ssm_disable_z
    ...
    # 修改后的 forward_features（兼容 channel_last）
    def forward_features(self, x: torch.Tensor):
        """
        返回每个 stage 的特征图（在下采样前），用于密集预测任务
        输出格式：List[Tensor]，每个 Tensor 为 (B, C, H', W')，channel_first
        """
        x = self.patch_embed(x)  # (B, H//4, W//4, C0) if not channel_first
        features = []

        for layer in self.layers:
            # Step 1: 通过 VSSBlocks（不包含 downsample）
            x = layer.blocks(x)  # 输出 (B, H', W', C) 或 (B, C, H', W') 取决于 channel_first

            # Step 2: 转为 channel_first 并保存
            if not self.channel_first:
                x_feat = x.permute(0, 3, 1, 2).contiguous()  # (B, H', W', C) -> (B, C, H', W')
            else:
                x_feat = x
            features.append(x_feat)

            # Step 3: 应用下采样到下一层（最后一层无下采样）
            if not isinstance(layer.downsample, (nn.Identity, type(None))):
