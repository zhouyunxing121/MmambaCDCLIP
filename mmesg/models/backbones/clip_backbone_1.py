...
#from models.backbones.vmamba import VSSM, PatchEmbed
from .vmamba import VSSM, PatchEmbed
@MODELS.register_module()
class CLIPMambaWithAttention_v0(nn.Module):
    def __init__(
        self,
        # Mamba Encoder 参数
        #patch_size=3,
        ...
        **kwargs
        ...
        """
        # === 使用 ChangeMamba 的 Patch Embedding ===
        self.patch_embed = PatchEmbed(
            patch_size=patch_size, in_chans=in_chans, embed_dim=dims[0], norm_layer=None
        )
        #VSSM 内部已经包含了它自己的 patch_embed！

        #所以模型实际上有 两个 patch_embed：

        #顶层：self.patch_embed
        #内部：self.mamba_encoder.patch_embed
        #但预训练权重只包含 mamba_encoder.patch_embed.proj.weight 这样的 key。

        #因此，顶层的 self.patch_embed 永远不会被使用，却导致 state_dict 多出无用的 key（如 patch_embed.proj.weight），造成 confusion。
        """
        
        # === Mamba Backbone (来自 ChangeMamba 的 VSSM) ===
    ...
    def init_weights(self, pretrained):
        ...
        state_dict = {
            ...
            #if not k.startswith('classifier') and not k.startswith('norm')
        ...
        # =============== 关键：扩展 patch_embed 卷积核（3 → 6 通道） ===============
        # 检查当前模型是否为 6 通道输入，而预训练是 3 通道
        #current_in_chans = self.mamba_encoder.patch_embed.proj.weight.shape[1]  # 当前实际 in_chans

        # 安全获取 patch_embed 的卷积层
        patch_embed = self.mamba_encoder.patch_embed
        # 初始化变量
        proj_layer = None
        proj_key = None
        if hasattr(patch_embed, 'proj'):
            proj_layer = patch_embed.proj
            proj_key = 'patch_embed.proj.weight'
        elif isinstance(patch_embed, nn.Sequential):
            proj_key = 'patch_embed.0.weight'  # <-- 这是关键！Sequential 的第一层是 '0'
            proj_layer = patch_embed[0]
        else:
            raise AttributeError("Cannot locate the projection conv layer in patch_embed!")
        
        # 获取当前模型的输入通道数
        #current_in_chans = proj_layer.weight.shape[1]
        current_in_chans = self.mamba_encoder.patch_embed[0].weight.shape[1] if isinstance(self.mamba_encoder.patch_embed, nn.Sequential) \
                   else self.mamba_encoder.patch_embed.proj.weight.shape[1]
        """
        if current_in_chans == 6:
            # 查找预训练中的 patch_embed 权重
            proj_key = 'patch_embed.0.weight'#proj_key = 'patch_embed.proj.weight'
            if proj_key in state_dict:
                pretrained_weight = state_dict[proj_key]  # shape: [embed_dim, 3, kernel, kernel]
                if pretrained_weight.shape[1] == 3:
                    # 将 3 通道权重复制一份，拼接成 6 通道
                    # 策略：简单复制（也可用均值、随机初始化等，但复制最稳定）
                    expanded_weight = torch.cat([pretrained_weight, pretrained_weight], dim=1)
                    state_dict[proj_key] = expanded_weight
                    print(f"Expanded patch_embed weight from 3 to 6 channels: {pretrained_weight.shape} -> {expanded_weight.shape}")
                elif pretrained_weight.shape[1] != 6:
                    print(f"Warning: Unexpected in_chans in pretrained weight: {pretrained_weight.shape[1]}")
            else:
                print("Warning: 'patch_embed.proj.weight' not found in pretrained state_dict.")
        # ======================================================================
        """
        if current_in_chans == 6:
        # 尝试两种可能的 key
            expanded = False
            if 'patch_embed.proj.weight' in state_dict:#patchembed_version="v2" (单层卷积)
                weight_key = 'patch_embed.proj.weight'
                pretrained_weight = state_dict[weight_key]
                if pretrained_weight.shape[1] == 3:
                    state_dict[weight_key] = torch.cat([pretrained_weight, pretrained_weight], dim=1)
                    print(f"[INFO] Expanded {weight_key} from 3 to 6 channels.")
                    expanded = True
            elif 'patch_embed.0.weight' in state_dict:#patchembed_version="v1" (多层 Sequential)
                weight_key = 'patch_embed.0.weight'
                pretrained_weight = state_dict[weight_key]
                if pretrained_weight.shape[1] == 3:
                    state_dict[weight_key] = torch.cat([pretrained_weight, pretrained_weight], dim=1)
                    print(f"[INFO] Expanded {weight_key} from 3 to 6 channels.")
                    expanded = True
        # 关键：给所有 backbone 权重加上 'mamba_encoder.' 前缀
        new_state_dict = {}
        for k, v in state_dict.items():
        # 所有来自 VSSM 的权重都属于 mamba_encoder
            new_state_dict[f'mamba_encoder.{k}'] = v

        # Remove classifier head if needed
        #state_dict = {k: v for k, v in state_dict.items()
         #         if not k.startswith('classifier') and 'head' not in k}
        ...
        state_dict = {k: v for k, v in state_dict.items() if not k.startswith('classifier') and not k.startswith('norm')}
        self.load_state_dict(state_dict, strict=False)
    ...
    def forward(self, x):
        ...
        # 使用 Mamba Encoder 提取多阶段特征 
        # ChangeMamba 的 VSSM.forward_features 返回各阶段输出 list
        # 每个元素 shape: (B, L, D) 或 (B, D, H', W')
        features = self.mamba_encoder.forward_features(x)  # List[Tensor], len=4

        outs = []
        for i, feat in enumerate(features):
            D = self.dims[i]
            S = H // (4 * (2 ** i))  # 因为 patch_size=4, 第一级下采样4倍，之后每级×2
            if feat.dim() == 3:  # (B, L, D)
                L = feat.size(1)
                assert L == S * S, f"Length mismatch at stage {i}: {L} != {S*S}"
                feat = feat.transpose(1, 2).view(B, D, S, S)  # (B, D, H', W')
            outs.append(feat)

        # === Attention Pooling on last feature ===
        x_last = outs[-1]  # (B, D, H', W')
        x_global, x_local = self.attnpool(x_last)
        outs.append([x_global, x_local])
...
from .vmamba import VSSM, PatchEmbed
@MODELS.register_module()
class CLIPMambaWithAttention_v1(nn.Module):
    def __init__(
        self,
        # Mamba Encoder 参数
        #patch_size=4,
        ...
        **kwargs
        ...
        print(">>> Expected patch_embed.0 out channels:", dims[0] // 2)
        # === Mamba Backbone (来自 ChangeMamba 的 VSSM) ===
        self.mamba_encoder = VSSM(
            patch_size=patch_size,
            in_chans=in_chans,
    ...
    def init_weights(self, pretrained):
       ...
       # === Step 2: 手动重命名 key ===
        ...
        expanded = False
        # 情况1: patchembed_version="v2" (单层卷积)
        if 'patch_embed.proj.weight' in state_dict:
            weight = state_dict['patch_embed.proj.weight']  # [out, in, k, k]
            #if weight.shape[1] == 3 and self.mamba_encoder.in_chans == 6:
            if weight.shape[1] == 3 and self.in_chans == 6:
                state_dict['patch_embed.proj.weight'] = torch.cat([weight, weight], dim=1)
                print(f"[INFO] Expanded patch_embed.proj.weight from 3 to 6 channels.")
                expanded = True
    
        # 情况2: patchembed_version="v1" (多层 Sequential)
        elif 'patch_embed.0.weight' in state_dict:
            weight = state_dict['patch_embed.0.weight']  # [out, in, k, k]
            #if weight.shape[1] == 3 and self.mamba_encoder.in_chans == 6:
            if weight.shape[1] == 3 and self.in_chans == 6:
...
CLIPMambaWithAttention	 6 通道单流（拼接 img1+img2 → 单 backbone）
• in_chans=6
• 实现了 3→6 权重扩展	—
ChangeCLIPCD.py	—	 3 通道双流（拆分 img1/img2 → 同一 backbone 调用两次）
• inputsA = inputs[:, :3]
• xA = self.extract_feat(inputsA)
结果：6 通道 backbone 从未被以 6 通道方式调用！它实际上是以 3 通道模式运行（如果没报错），导致：

3→6 权重扩展逻辑未被触发；
Mamba 的跨时相建模能力未被激活；
架构混乱，无法发挥设计优势。

据调查，6 通道拼接的单流架构（Early Fusion）在性能、效率、实现简洁性和学术前沿性上全面优于传统双流 Siamese 架构，
已成为当前变化检测领域的绝对主流。
另外，使用 CNN（如 ResNet）	双流 or 单流均可，性能接近；
但使用 Transformer / Mamba / ViT，双流（Siamese）无法发挥全局建模优势，必须用 6 通道单流。

本文件夹用于——————修改CLIPMambaWithAttention，适应并使用双流（Siamese）。
    """
    def __init__(
        self,
        # Mamba Encoder 参数
