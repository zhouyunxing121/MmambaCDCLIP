...
# noqa: E501
    ...
    def __init__(self,
        ...
        self.minus_conv = nn.ModuleList([
            ConvModule(in_channels=ch, out_channels=score_channels, kernel_size=1)
            for ch in actual_feat_channels  # 或者使用 x_clip_channels，取决于实际拼接了 score_map 没有
        ])
       ...
       #2026-4-9-修改——————见————————Gemini——————ChangeCLIP 项目介绍与解析
        if self.mamba_layers:
            ...
            # 融合后 Mamba：在 768 维的特征空间中进行全局序列扫描
            self.mamba_modules_fused = nn.ModuleList([
...
#2026-4-9-修改——————见————————Gemini——————ChangeCLIP 项目介绍与解析
    ...
    def encode_decode(self, inputs: Tensor,
        ...
        #    _x_orig = x_orig
        # 2. 先过 FPN (Neck)：处理 x_orig，将其多尺度融合并统一降维到 256
        if getattr(self, 'with_neck', False):
            # 注意：配置文件中的 neck.in_channels 必须恢复为 backbone 的输出维度 (如 [192, 384, 768, 1540])
            x_orig = list(self.neck(x_orig)) 
            _x_orig = x_orig  # 供 aux_head 使用
        losses = dict()
        ...
        # 3. 构建 768 维的强语义特征
        # 公式: x[i] (256) * x_diff (1) + x_minus (256) + x[i] (256) -> [B, 768, H, W]
        x = [torch.cat([x[i]*x_diff[i], x_minus[i], x[i]], dim=1) for i in range(len(x))]
        # 4. Mamba 全局空间扫描 (在 768 维空间)
    ...
    def encode_decode(self, inputs: Tensor,
        ...
        # 1. 特征提取
        text_embeddingsA, x_clipA_raw, _, _ = self.after_extract_feat_clip(xA, textA)
        text_embeddingsB, x_clipB_raw, _, _ = self.after_extract_feat_clip(xB, textB)

        # 2. 纯视觉线 -> FPN
        x_orig_raw = [
            torch.cat([x_clipA_raw[i], x_clipB_raw[i]], dim=1) 
            for i in range(len(x_clipA_raw))
        ]
        
        if getattr(self, 'with_neck', False):
            x_neck = list(self.neck(x_orig_raw))
        else:
            x_neck = x_orig_raw

        if getattr(self, 'text_head', False):
            x = [text_embeddingsA,] + x_neck
        else:
            x = x_neck
        ...
        # 4. 构建 768 维特征
        x_semantic = [torch.cat([x[i]*x_diff[i], x_minus[i], x[i]], dim=1) for i in range(len(x))]

        # 5. Mamba 扫描
        if getattr(self, 'mamba_layers', False):
            x_fused = []
            for i, feat in enumerate(x_semantic):
                seq = self.spatial_to_sequence(feat)
                seq = self.mamba_modules_fused[i](seq)
                h, w = feat.shape[2], feat.shape[3]
                feat = self.sequence_to_spatial(seq, h, w)
        ...
        # 6. SELayer 降维
        x_final = [self.channel_att[i](x_semantic[i]) for i in range(len(x_semantic))]

        # 7. 解码输出
        seg_logits = self.decode_head.predict_with_text(x_final, text_embeddingsA, text_embeddingsB, batch_img_metas, self.test_cfg)
...
#2026-4-6-修改——————见————————Gemini——————ChangeCLIP 项目介绍与解析
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        # 1. 获取增强后的文本特征，以及原始(raw)特征
        text_embeddingsA, x_clipA_raw, _, score_mapA = self.after_extract_feat_clip(xA, textA)
        text_embeddingsB, x_clipB_raw, _, score_mapB = self.after_extract_feat_clip(xB, textB)
        
        score_map_diff = score_mapA - score_mapB

        # 2. 【视觉线】用 raw 版本做基础拼接 -> 送入 FPN
        # 此时的 x_orig_raw 的通道数严格等于 2 * actual_feat_channels (即 192, 384, 768, 1536)
        x_orig_raw = [
            torch.cat([x_clipA_raw[i], x_clipB_raw[i]], dim=1) 
            for i in range(len(x_clipA_raw))
        ]
        ...
        if getattr(self, 'with_neck', False):
            # FPN 接收 [192, 384, 768, 1536]，输出 4 层 256 维
            x_neck = list(self.neck(x_orig_raw))
            _x_orig = x_neck  # 供 aux_head 使用
        else:
            x_neck = x_orig_raw
            _x_orig = x_neck
        ...
        # 3. 【多模态语义线】提取差异先验
        # x_minus: 提取多尺度差值特征 (输出全是 256 维)
        x_minus = [self.minus_conv[i](torch.abs(x_clipA_raw[i] - x_clipB_raw[i])) for i in range(len(x_clipA_raw))]
        
        # x_diff: 提取多尺度相似度权重 (输出全是 1 维)
        x_diff = [
            F.sigmoid(1 - torch.cosine_similarity(x_clipA_raw[i], x_clipB_raw[i], dim=1)).unsqueeze(1) 
            for i in range(len(x_clipA_raw))
        ]
        ...
        # 4. 构建 768 维的强语义特征
        # 公式: x_neck[i] (256) * x_diff (1) + x_minus (256) + x_neck[i] (256) -> [B, 768, H, W]
        x_semantic = [torch.cat([x[i]*x_diff[i], x_minus[i], x[i]], dim=1) for i in range(len(x))]

        # 5. Mamba 全局空间扫描 (在 768 维空间)
        if getattr(self, 'mamba_layers', False):
            x_fused = []
            for i, feat in enumerate(x_semantic):
                seq = self.spatial_to_sequence(feat)
        ...
        # 6. 通道注意力与降维 (从 768 提纯并降回 256)
        x_final = [self.channel_att[i](x_semantic[i]) for i in range(len(x_semantic))]

        # --- 准备计算 Loss ---
        losses = dict()

        loss_decode = self._decode_head_forward_train_with_text(x_final, text_embeddingsA, text_embeddingsB, data_samples)
        losses.update(loss_decode)
        ...
        if self.with_identity_head:
            loss_identity_sm = self._identity_head_forward_train(
                score_map_diff/self.tau, data_samples, 'aux_score_map')
            losses.update(loss_identity_sm)
            loss_identity1 = self._identity_head_forward_train(x_final[0], data_samples, 'aux_layer0')
            losses.update(loss_identity1)
            loss_identity2 = self._identity_head_forward_train(x_final[1], data_samples, 'aux_layer1')
            losses.update(loss_identity2)
            loss_identity3 = self._identity_head_forward_train(x_final[2], data_samples, 'aux_layer2')
            losses.update(loss_identity3)
            loss_identity4 = self._identity_head_forward_train(x_final[3], data_samples, 'aux_layer3')
            losses.update(loss_identity4)
...
#2026-4-9-修改——————见————————Gemini——————ChangeCLIP 项目介绍与解析
    ...
    def _forward(self,
        ...
        data_samples = [{'image_shape': (256, 256)}]
        seg_logits = self.decode_head.predict_with_text(x_final, text_embeddingsA, text_embeddingsB, data_samples, self.test_cfg)
