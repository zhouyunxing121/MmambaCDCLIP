...
# noqa: E501
    ...
    def __init__(self,
                 ...
                 text_dim=768,
                 #text_dim=384,
                 #minus_channel = [256, 512, 1024, 2050],#minus_channel 可删除,不再使用（已被 actual_feat_channels 替代）
                 minus_channel = [96, 192, 384, 770],
        ...
        #   backbone 的预训练由 backbone 自己的 pretrained_mamba 控制
            if any(kw in pretrained for kw in ['RN50', 'RN101', 'ViT-B', 'clip']):
            ...
            print("No pretrained provided. Using default CLIP RN50 for text encoder.")
            text_encoder_pretrained = '/home/dc001/.cache/clip/RN50.pt'
        ...
        self.minus_channel = minus_channel#minus_channel 可删除,不再使用（已被 actual_feat_channels 替代）
        ...
        if self.mamba_layers:
            self.mamba_modules_fused = nn.ModuleList([
             MambaLayer(dim=ch,  d_state=mamba_d_state, d_conv=mamba_d_conv, expand=mamba_expand) for ch in fused_channels
            ])
            #dim=2*ch + 256
        #2026-1-11修改，见——看看原ChangeCLIP项目怎么解决的
