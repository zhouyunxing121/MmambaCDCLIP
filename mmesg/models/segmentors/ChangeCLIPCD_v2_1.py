...
# noqa: E501
    ...
    def __init__(self,
        ...
        # 引入 ProText 辅助增强模块
        # ==========================================
        #self.use_protext = protext_feat_path is not None
        #if self.use_protext:
            # 1. 尝试加载 ProText 专家特征
        #    try:
        ...
        #        
        #        # 2. 定义可学习的融合参数 (初始值为 0.0，Sigmoid 后为 0.5)
        #        self.protext_alpha_raw = nn.Parameter(torch.tensor(0.0))
        #        print(f" Successfully loaded ProText features from {protext_feat_path}")
    ...
    def encode_decode(self, inputs: Tensor,
        ...
        #x[i]：原始拼接特征。
        #x = [torch.cat([x[i]*x_diff[i], x_minus[i], x[i]], dim=1) for i in range(len(x))]
        # 5. 高维超级特征拼接 (256 + 256 + 256 = 768 维)
        x = [torch.cat([x[i]*x_diff[i], x_minus[i], x[i]], dim=1) for i in range(len(x))]
        #2026-4-9-修改——————见————————Gemini——————ChangeCLIP 项目介绍与解析
        #将 Mamba 提前：在高维空间中扫描，理解全局的建筑物分布
        #if self.mamba_layers:
        #    x_fused = []
        #    for i, feat in enumerate(x):
        #        seq = self.spatial_to_sequence(feat)
        #        # 此时的 feat 维度是 fused_channels[i]
        #        seq = self.mamba_modules_fused[i](seq)
        #        h, w = feat.shape[2], feat.shape[3]
        #        feat = self.sequence_to_spatial(seq, h, w)
        #        x_fused.append(feat)
...
#2026-4-6-修改——————见————————Gemini——————ChangeCLIP 项目介绍与解析
    ...
    def _forward(self,
        ...
        if data_samples is None:
            # 动态根据输入张量生成 dummy 的 data_samples
            data_samples = [
                dict(
                    ori_shape=inputs.shape[2:],
                    img_shape=inputs.shape[2:],
